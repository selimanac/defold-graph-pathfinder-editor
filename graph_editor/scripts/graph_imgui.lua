...
function graph_imgui.init()
	imgui.set_display_size(sys.get_config_int("display.width", 1920), sys.get_config_int("display.height", 1080)) -- TODO get from project file
...
local function stats()
	...
	if data.stats.path_cache then
		local cache = data.stats.path_cache
		imgui.text(string.format("Path Cache - Current Entries: %d | Max Capacity: %d | Hit Rate: %d%%",
			cache.current_entries, cache.max_capacity, cache.hit_rate))
	end

	if data.stats.distance_cache then
		local cache = data.stats.distance_cache
		imgui.text(string.format("Distance Cache - Current Entries: %d | Hit Count: %d | Miss Count: %d | Hit Rate: %d%%",
			cache.current_size, cache.hit_count, cache.miss_count, cache.hit_rate))
	end

	if data.stats.spatial_index then
		local index = data.stats.spatial_index
		imgui.text(string.format("Spatial Index - Cell Count: %d | Edge Count: %d | Avg. Edges per Cell: %d | Max Edges per Cell: %d",
			index.cell_count, index.edge_count, index.avg_edges_per_cell, index.max_edges_per_cell))
	end
	imgui.separator()
...
local function node()
	...
	imgui.text("UUID: " .. data.selected_node.uuid)
	...
	if changed then
		-- Set coordinates based on plane type to avoid redundant operations
		...
		if const.IS_XZ_PLANE then
			...
			data.selected_node.position.z = 0
		end
...
local function settings()
	...
	if paths_tab_open then
		...
		imgui.separator()
		if imgui.begin_combo("AGENT MODE##selectable", get_key_for_value(const.AGENT_MODE, data.agent_mode)) then
			for key, mode in pairs(const.AGENT_MODE) do
		...
		if changed then
			...
			if const.IS_XZ_PLANE then
				...
				data.options.projected_to_projected.start_position.y = y
				data.options.projected_to_projected.start_position.z = 0
			end
		end
	...
	-- SMOOTHING
	-- =======================================
	local smoothing_tab_open = imgui.begin_tab_item("Smoothing")
	if smoothing_tab_open then
		imgui.set_next_item_width(250)
